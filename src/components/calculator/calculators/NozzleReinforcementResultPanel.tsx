...
type NozzleReinforcementResultPanelProps = {
  output: CalculatorOutput;
  exportTitle: string;
  standard?: string;
  inputRows: { label: string; value: string }[];
  /** Hero → schematic → detail tables (FEK chart placement). */
  afterHero?: ReactNode;
};
...
export default function NozzleReinforcementResultPanel({
  output,
  exportTitle,
  standard,
  inputRows,
  afterHero,
}: NozzleReinforcementResultPanelProps) {
  ...
  return (
    <div className="flex w-full min-w-0 max-w-full flex-col space-y-3.5">
      ...
      {afterHero ? (
        <div className="min-w-0 [&_.schematic-frame]:mt-0 [&_.schematic-frame]:border-0 [&_.schematic-frame]:pt-0 [&_svg]:mx-auto [&_svg]:h-auto [&_svg]:w-full">
          {afterHero}
        </div>
      ) : null}
