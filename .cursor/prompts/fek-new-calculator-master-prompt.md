...
참조 계산기 (복제 템플릿, 필수 1개 이상):
  ...
  - Engine/pSEO: (예: bolt-torque / multi-pump)
기본 듀티 (defaults):
pSEO 예시 케이스 (metric/imperial 각 ≥2, 실제 표 수치와 일치):
  - spec 경로:
    query:
    기대 hero/핵심값:
...
- 완료 전 최소 게이트: 해당 파일 `tsc` 관련 오류 없음 + `npm run test -- tests/calculators.test.ts -t "<type-or-slug>"`
...
- [ ] `src/lib/calculators/url-sync.ts` — `PATH_OWNED_PARAMS`에 path 키 추가
...
- [ ] `data/calculatorSeoData.ts` — **완전** `CalculatorSeoEntry` (아래 스키마)
...
- [ ] `tests/calculators.test.ts` — engine 수치 + Pattern B resolve/findSpec 테스트
...
`data/calculatorSeoData.ts` 키 = **공개 슬러그**. 최소 포함:

- `slug`, `formulaTitle`, `formulaHtml`, `formulaLatex`, `formulaNotes`
- `formulaBadges` (권장)
- `variables[]`, `standards[]`
- `tableCaption`, `tableHeaders`, `tableRows`, `tableFootnote?`
- `allowancesAndTolerances?`, `materialLimitations?`, `workedExample?`
...
### Hero
- 현장 1초 조회 값 = hero (필요하면 **양 단위 동시** 표기, 예: `1-1/4 in · 32 mm`)
- `heroBadges`는 보조만 (hero와 중복 나열 최소화)
- `callouts`: Heavy hex / screening / OEM chart 등 도메인 경고
...
2. `CalculatorSeoContent` — formula / lookup table / how-to / FAQ / JSON-LD  

Agent 할 일:
- Spec별 카피는 **`buildSpecSeoCopy` 분기**로 넣는다
- 하단 long-tail은 **`calculatorSeoData` workedExample + faq + tableRows`**로 흡수한다
...
1. Engine + 단위 헬퍼 + **표 교차검증** (worked example 수치를 코드로 assert)  
2. url-config + pseo routes  
3. UI (참조 계산기 복제) + url sync  
4. definitions → Shell → tags → bindings → PATH_OWNED_PARAMS → spec-routes  
5. localSeed + calculatorSeoData(howTo·faq 포함) + home ui/workstation + redirect  
6. tests  
...
- [ ] `tests/calculators.test.ts` 해당 describe 통과
- [ ] hero/차트/입력이 중복 카피로 지저분하지 않음
...
```text
FEK Pattern B 신규 계산기 — `.cursor/prompts/fek-new-calculator-master-prompt.md` 준수, 스펙 기입란 기준으로 zero-rework 등록. 그리드 변경·sitemap 수동·canonical query 금지.
```
